...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| XFlow | PROVEN | PROVEN | PROVEN | BLOCKED | NOT RUN | PARTIAL | PARTIAL | NOT RUN | Railway deployment for approved commit `960b5eff27f07b8bb9db83146422095855d7feec` failed at `BUILD_IMAGE`; Railway finite log batches exposed no actionable build log lines. Local install, typecheck, Next build, forced standalone build, Docker deps-stage install simulation, and focused tests passed. Production still serves last successful commit `af5a494da0c1b292815dffcd771e663f7cc76751`. Authenticated proof must wait for drift closure. | 2026-07-11 | `docs/production-proof/2026-07-11-xflow-public-deployment-proof.md`; Railway deployment `673f961e-8bcb-40c8-8612-0f4290f256d4`; `npm run build`; `npm run typecheck`; `curl https://xflowx.com/api/health`; `curl https://xflowx.com/api/ready` |
