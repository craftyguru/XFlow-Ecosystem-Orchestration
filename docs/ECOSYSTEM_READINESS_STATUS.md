...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| XFlow | PROVEN | PROVEN | PROVEN | BLOCKED | NOT RUN | PARTIAL | PARTIAL | NOT RUN | Railway deployment for approved commit `960b5eff27f07b8bb9db83146422095855d7feec` failed at `BUILD_IMAGE` because XFlow imported Verixet's generated public catalog from a sibling repository path that is unavailable in Railway's standalone build context. Fix commit `965989a165926ce1de40e6353d6140a45a636d16` replaces that cross-repo runtime dependency with an in-repo generated mirror, but production still serves last successful commit `af5a494da0c1b292815dffcd771e663f7cc76751` until a new approved deployment proves the fix. Authenticated proof must wait for drift closure. | 2026-07-11 | `docs/production-proof/2026-07-11-xflow-public-deployment-proof.md`; Railway deployment `673f961e-8bcb-40c8-8612-0f4290f256d4`; `npm run verify:standalone-catalog-mirror`; `npm run build`; `npm run typecheck`; `curl https://xflowx.com/api/health`; `curl https://xflowx.com/api/ready` |
...
3. Redeploy XFlow fix commit `965989a165926ce1de40e6353d6140a45a636d16`, then verify public health/readiness reports that exact HEAD.
